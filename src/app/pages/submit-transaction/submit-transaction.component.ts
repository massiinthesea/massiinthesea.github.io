...
export class SubmitTransactionComponent implements OnInit {
  ...
  //Select Lists
  accountsToDebitList: Account[] = Account.generateList();
  allCurrenciesList: Currency[] = Currency.allCurrencies;
  constructor(
  ...
  private initFormGroup() {
    this.transactionForm = this.fb.group({
      ...
      currency: [this.allCurrenciesList[0]],
      feePaidByReceiver: [false],
      destinationAccount: [null, [Validators.required]],
      metadata: this.fb.array([])
    });
    this.addMetaDataControl();

    //TODO: unsubscribe ?
    this.transactionForm.get("metadata").valueChanges.subscribe(metaDatas => {
      this.metadataFieldsChange(metaDatas);
    })
  }

  addMetaDataControl(): void {
    this.metadataFormArray.push(this.newMetadataControlGroup(false));
  }

  newMetadataControlGroup(requiredField?): FormGroup {
    return this.fb.group({
      key: requiredField ? [null, [Validators.required]] : '',
      value: requiredField ? [null, [Validators.required]] : '',
    })
  }
  get metadataFormArray(): FormArray {
    return this.transactionForm.get("metadata") as FormArray
  }

  metadataFieldsChange(metadatas) {
    let shouldAddMetadataControl: boolean = true;
    metadatas.forEach(metadata => {
      if (!this.isValideMetadata(metadata)) {
        shouldAddMetadataControl = false;
      }
    });
    shouldAddMetadataControl ? this.addMetaDataControl() : null;
  }

  resetForm(): void {
    this.transactionForm.reset();
    this.transactionForm.controls["currency"].setValue(this.allCurrenciesList[0]);
    this.metadataFormArray.controls = [];
    this.addMetaDataControl();
  }

  validMetaDataForm(): boolean {
    return this.metadataFormArray.value.find(metaData => this.isValideMetadata(metaData)) === -1 ? false : true;
  }

  removeMetaDataControl(i: number): void {
    this.metadataFormArray.removeAt(i);
  }

  isMetaDataLineDeletable(i: number): boolean {
    return !this.isValideMetadata(this.metadataFormArray.value[i]);
  }

  isValideMetadata(metadata): boolean {
    return (metadata.key && metadata.key !== "" && metadata.value && metadata.value !== "")
  }

  submitForm(): void {
    this.modalService.confirm({
      nzTitle: 'Are you sure to submit this transaction?',
      nzContent: '<b style="color: red;">Some descriptions</b>',
      nzOkText: 'Yes',
      nzOkType: 'primary',
      // nzOkDanger: true,
      nzOnOk: () => this.saveTransaction(),
      nzCancelText: 'No',
  ...
  saveTransaction(): void {
    const transaction = new Transaction(this.transactionForm.value);
    const transactionCreated = this.localStorageService.saveTransaction(transaction);
    if (transactionCreated) {
      this.nzMessageService.success('Transaction submited.');
      this.resetForm();
    } else {
      this.nzMessageService.error('A problem arrived, transaction has not been submited.');
  ...
  initLesCurrencies() {
    const currencies = this.getCurrenciesJson();
    const currenciesStringLines = [];
    currencies.forEach(currencie => {
      currenciesStringLines.push(
        `public static readonly ${currencie.code.toUpperCase() + '_' + currencie.name.toLocaleUpperCase().split(' ').join('_').split('-').join("_")} = new Currency( '${currencie.code}', '${currencie.name}', '${currencie.symbolNative}');`
      )
    })
    const stringSaMere = currenciesStringLines.join("\n");
    console.log("owwwwww  =====> \n \n", stringSaMere);
  }
